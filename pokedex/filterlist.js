...
function filterlist(selectobj) {
  ...
  this.set = function(pattern) {
    ...
    // Clear the select list so nothing is displayed
     if (!this.searchorfind) this.selectobj.options.length = 0;
	...
	if (this.searchorfind) // Just a little modification by Improfane to actually not erase the options contents, only highlight it
	{ this.selectobj.options[loop].selected = 1 ; break ;}
        else this.selectobj.options[index++] =
